/**
 * Example demonstrating Redis as primary storage for BrowserState
 * This example shows how to use Redis directly as the storage backend
 * instead of just as a caching layer
 */
...
const REDIS_CONFIG = {
  ...
  // Advanced options
  maxFileSize: 20 * 1024 * 1024, // 20MB per file to handle Chrome profile files
  ...
  try {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('not found')) {
        ...
        // Create a new temporary directory for the session
        const tempDir = path.join(os.tmpdir(), `browserstate-${sessionId}-${Date.now()}`);
        fs.mkdirSync(tempDir, { recursive: true });
        
        console.log(`📂 Created new temporary directory: ${tempDir}`);
